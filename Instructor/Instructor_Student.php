...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['full_name']) && isset($_POST['email'])) {
    ...
    $accessibility = htmlspecialchars($_POST['accessibility'] ?? 'user'); // default user role

    $stmt = $pdo->prepare("INSERT INTO users (name, email, role, gender, phone_number, password) VALUES (?, ?, ?, '', '', '')");
    $stmt->execute([$full_name, $email, $accessibility]);

    echo "<script>alert('Student added successfully!'); window.location.href=window.location.href;</script>";
    exit;
}

// Fetch enrolled students related to this instructor
$stmt = $pdo->prepare("
                ...
                foreach ($students as $student) {
                    ...
                    echo "<td data-label='Actions'>
                            <div class='student-actions'>
                                <button class='action-btn delete-btn' onclick='deleteStudent(" . (int)$student['student_id'] . ")'>
        ...
        <form id="studentForm" method="POST">
            ...
            <div class="form-row">
                ...
                <div class="accessibility-options">
                    <div class="accessibility-option" onclick="selectAccessibility('deaf')">
                        <i class="fas fa-deaf"></i> Deaf/HoH
                    </div>
                    <div class="accessibility-option" onclick="selectAccessibility('blind')">
...
// Simple search/filtering (optional)
